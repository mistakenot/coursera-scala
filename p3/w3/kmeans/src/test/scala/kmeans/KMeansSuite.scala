...
class KMeansSuite extends FunSuite {
  ...
  test("'kMeans' should work for 'points' == GenSeq((0, 0, 1), (0,0, -1), (0,1,0), (0,10,0)) and 'oldMeans' == GenSeq((0, -1, 0), (0, 2, 0)) and 'eta' == 12.25") {
    ...
    val eta = 12.25f
    val actual = kMeans(points, means, eta).map(p => (p.x, p.y, p.z)).toList
    assert(actual == List((0.0, 0.0, 0.0), (0.0, 5.5, 0.0)))
  }

  test("'converged' should work for empty oldMeans and empty newMeans") {
    converged(1.00)(GenSeq.empty, GenSeq.empty)
  }
  ...
  test("'update' should work") {
    ...
    val means = GenSeq(new Point(0, -1, 0), new Point(0, 2, 0))
    val eta = 12.25f
    assert(update(classify(points, means), means).map(p => (p.x, p.y, p.z)) == List((0.0, 0.0, 0.0), (0.0, 5.5, 0.0))) 
  }
